...
@Service
public class CurveTemperatureService implements IMonitorService {
    @Autowired
    ...
    public List<CommonMonitorModel> getEnvMonitorInTimeQuery(EnvQueryInTimeParam envQueryInTimeParam) {
        return commonMonitorDao.getInTimeData(new ModelParamBo(CommonConstant.TABLE_CURVE_TEMPERATURE, envQueryInTimeParam.getPointArr()));
    }

    @Override
    public List<CommonMonitorModel> getEnvMonitorHistoryQuery(EnvQueryHistoryParam envQueryHistoryParam) {
        return commonMonitorDao.getMonitorHistory(new ModelParamBo(CommonConstant.TABLE_CURVE_TEMPERATURE,
    ...
    public int batchInsert(List<MonitorUploadParam> monitorUploadParamList) {
        return commonMonitorDao.batchInsert(new InsertModelParamBo(CommonConstant.TABLE_CURVE_TEMPERATURE,monitorUploadParamList));
    }

    @Override
    public void cleanEnvData(String cleanTime) {
        commonMonitorDao.cleanMonitorData(new CleanParamBo(cleanTime,CommonConstant.TABLE_CURVE_TEMPERATURE));
    }
}