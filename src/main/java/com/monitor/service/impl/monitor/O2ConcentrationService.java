...
@Service
public class O2ConcentrationService implements IMonitorService {
    ...
    public List<CommonMonitorModel> getEnvMonitorInTimeQuery(EnvQueryInTimeParam envQueryInTimeParam) {
        return commonMonitorDao.getInTimeData(new ModelParamBo(CommonConstant.TABLE_O2_CONCENTRATION, envQueryInTimeParam.getPointArr()));
    }

    @Override
    public List<CommonMonitorModel> getEnvMonitorHistoryQuery(EnvQueryHistoryParam envQueryHistoryParam) {
        return commonMonitorDao.getMonitorHistory(new ModelParamBo(CommonConstant.TABLE_O2_CONCENTRATION,
    ...
    public int batchInsert(List<MonitorUploadParam> monitorUploadParamList) {
        return commonMonitorDao.batchInsert(new InsertModelParamBo(CommonConstant.TABLE_O2_CONCENTRATION,monitorUploadParamList));
    }

    @Override
    public void cleanEnvData(String cleanTime) {
        commonMonitorDao.cleanMonitorData(new CleanParamBo(cleanTime,CommonConstant.TABLE_O2_CONCENTRATION));
    }
}